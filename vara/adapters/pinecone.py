"""
vara.adapters.pinecone — Pinecone adapter (SDK v3+).

Install: pip install vara[pinecone]

Supports:
  - Serverless and pod-based Pinecone indexes
  - Namespace-scoped queries
  - Vara canonical filter → Pinecone metadata-filter translation
"""
...
try:
    ...
    raise ImportError(
        ...
        "Install it with: pip install vara[pinecone]"
    ) from exc
...
def _translate_filter(filters: dict[str, Any]) -> dict[str, Any]:
    """
    Translate a Vara canonical filter dict → Pinecone metadata filter dict.

    Pinecone uses a MongoDB-style filter format. The translation is mostly
    1:1 except:
      - Direct equality {"field": val} → {"field": {"$eq": val}}
      - Vara $not → Pinecone $nor (Pinecone doesn't support top-level $not)
...
class PineconeAdapter(VecDBAdapter):
    ...
    async def list_collections(self) -> list[CollectionInfo]:
        """List all Pinecone indexes as Vara collections."""
            ...
            return HealthReport(
                ...
                findings=[
                    HealthFinding(
                        ...
                        detail=str(exc),
                        recommendation=(
                            "Check that your api_key in vara.yaml is valid "
            ...
            return HealthReport(
                ...
                findings=[
                    HealthFinding(
                        ...
                        recommendation=(
                            "Check the index name in vara.yaml or create the index first."
                        ),
                    )
                ],
                latency_ms=round((time.perf_counter() - t0) * 1000, 3),
