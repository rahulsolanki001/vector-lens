"""
vara.adapters.milvus — Milvus / Zilliz Cloud adapter.

Install: pip install vara[milvus]
...
Supports:
  ...
  - Vara canonical filter → Milvus expression-string translation
"""
...
try:
    ...
    raise ImportError(
        ...
        "Install it with: pip install vara[milvus]"
    ) from exc
...
def _build_expr(filters: dict[str, Any]) -> str:
    """
    Translate a Vara canonical filter dict → Milvus expression string.
            ...
            return HealthReport(
                ...
                findings=[
                    HealthFinding(
                        ...
                        detail=str(exc),
                        recommendation=(
                            "Check that Milvus is running and the host/port in vara.yaml are correct. "
            ...
            return HealthReport(
                ...
                findings=[
                    HealthFinding(
                        ...
                        recommendation="Check the collection name in vara.yaml.",
                    )
                ],
                latency_ms=round((time.perf_counter() - t0) * 1000, 3),
            )
