...
def eval_cmd(
    ...
    backend: str = typer.Option(..., "--backend", "-b", help="Backend name from vara.yaml"),
    collection: str = typer.Option(..., "--collection", help="Collection name"),
    config: str = typer.Option("vara.yaml", "--config", "-c", help="Path to vara.yaml"),
    output: str = typer.Option("vara_eval_results.json", "--output", "-o", help="Output JSON file"),
