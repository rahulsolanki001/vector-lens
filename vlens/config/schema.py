"""
vlens.config.schema — Pydantic models for vlens.yaml.
...
class VlensSettings(BaseModel):
    """
    Top-level [vlens] section of vlens.yaml.

    Controls the local server behaviour. All fields have sensible defaults
    so the entire section can be omitted from vlens.yaml.
    """

    model_config = {"extra": "forbid"}  # typos in vlens.yaml become errors, not silent ignores

    port: int = Field(
        default=7842,
        description="Port for the Vector Lens local server.",
    )
    open_browser: bool = Field(
        default=True,
        description="Automatically open the browser when `vlens serve` starts.",
    )
    log_level: str = Field(
        default="info",
...
class BackendConfig(BaseModel):
    """
    A single entry under the [backends] list in vlens.yaml.
...
class VlensConfig(BaseModel):
    """
    Root config model — the full contents of vlens.yaml.

    Fields:
        vlens:    Server settings (optional, all defaults are sensible).
        backends: List of DB backend connections (at least one required to use
                  `vlens serve`, but optional for programmatic SDK use).
    """

    model_config = {"extra": "forbid"}

    vlens: VlensSettings = Field(
        default_factory=VlensSettings,
        description="Global Vector Lens server settings.",
    )
    ...
    def validate_no_empty_backends(self) -> VlensConfig:
        """
    ...
    def get_backend(self, name: str) -> BackendConfig:
        ...
        Args:
            name: The backend name as defined in vlens.yaml.
