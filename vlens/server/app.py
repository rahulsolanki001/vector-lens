...
Responsibilities:
  - Load vlens.yaml and connect all configured adapters at startup
  ...
  - Serve the bundled React UI from vlens/server/static/ (if present)
"""
...
from vlens.server.routes import collections, config, query
from vlens.server.routes import eval as eval_routes
from vlens.server.routes import projection as projection_routes
...
def create_app(config_path: str | None = None) -> FastAPI:
    """
    Create and configure the Vector Lens FastAPI application.

    Args:
        config_path: Path to vlens.yaml. Reads VLENS_CONFIG env var if not
            provided, falling back to "vlens.yaml" in the current directory.
    """
    path = config_path or os.environ.get("VLENS_CONFIG", "vlens.yaml")
    ...
    try:
        ...
        extra_origins = cfg.vlens.cors_origins
    except Exception:
        pass
...
# Module-level app instance for `uvicorn vlens.server.app:app`
app = create_app()
