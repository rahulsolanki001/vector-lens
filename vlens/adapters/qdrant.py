"""
vlens.adapters.qdrant — Qdrant adapter.

Install: pip install vector-lens[qdrant]
...
try:
    ...
    raise ImportError(
        ...
        "Install it with: pip install vector-lens[qdrant]"
    ) from e
...
def _translate_filter(filters: dict[str, Any] | None) -> qmodels.Filter | None:
    """
    Translate Vector Lens canonical filter format → Qdrant Filter model.

    Vector Lens canonical format (mirrors a simple MongoDB-style DSL):
...
class QdrantAdapter(VecDBAdapter):
    ...
    async def query(self, request: QueryRequest) -> QueryResult:
        ...
        Translates Vector Lens canonical filters → Qdrant Filter, records the
            ...
            return HealthReport(
                ...
                findings=[
                    HealthFinding(
                        ...
                        detail=str(exc),
                        recommendation=(
                            "Check that Qdrant is running and the host/port in vlens.yaml are correct."
                        ),
                    )
                ],
