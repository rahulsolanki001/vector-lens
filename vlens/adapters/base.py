"""
vlens.adapters.base — Abstract base class and shared data models for all DB adapters.
...
# ── Config models (mirrors vlens.yaml backend entries) ────────────────────────
...
class QueryRequest(BaseModel):
    ...
    vector: list[float]
    top_k: int = 10
    filters: dict[str, Any] | None = None  # Vector Lens canonical filter format
...
class VecDBAdapter(ABC):
    """
    Abstract base class for all Vector Lens vector DB adapters.
    ...
    @abstractmethod
    def name(self) -> str:
        """The user-defined name from vlens.yaml (e.g. 'local-qdrant')."""
        pass
    ...
    async def health(self, collection: str) -> HealthReport:
        ...
        vlens.core.rules so findings are comparable across backends.
        """
        pass
