"""
vlens.adapters.pgvector — asyncpg + pgvector adapter.

Install: pip install vector-lens[pgvector]

Supports:
  - HNSW and IVFFlat indexes (auto-detected from pg_indexes)
  - Cosine (<=>), dot product (<#>), and L2 (<->) distance metrics
  - Vector Lens canonical filter translation → parameterised SQL WHERE clause
"""
...
try:
    ...
    raise ImportError(
        "asyncpg is required for the pgvector adapter.\nInstall it with: pip install vector-lens[pgvector]"
    ) from exc
...
# pgvector index operator class → Vector Lens distance metric name
...
def _build_where(
    ...
    """
    Translate Vector Lens canonical filter dict → SQL WHERE fragment.
        ...
        async with self._pool.acquire() as conn:
            ...
            if not txt_col_exists:
                raise RuntimeError(
                    ...
                    f"Set the correct column name via text_column in vlens.yaml."
                )
            ...
            return HealthReport(
                ...
                findings=[
                    HealthFinding(
                        ...
                        detail=str(exc),
                        recommendation="Check the DSN in vlens.yaml and that PostgreSQL is running.",
                    )
                ],
            ...
            if not table_exists:
                findings.append(
                    HealthFinding(
                        ...
                        recommendation="Check the 'table' field in vlens.yaml.",
                    )
                )
                return HealthReport(
                    backend_name=self.name,
